...
Posts.prototype.getPosts =  function(connection, callback){
    connection.query('select * from posts ORDER BY date DESC ', callback);
}

Posts.prototype.getPostage = function(connection, callback) {
    connection.query('select * from posts where id = 3', callback);       
}

Posts.prototype.savePostage = function(newpost, connection, callback){
